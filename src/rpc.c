#include "../include/rpc.h"
#include <stdlib.h>
#include <string.h>


//TODO: Is there any way I can make generating this Serialize easier? So I don't have to make the Args and Ret together? Is there any macro I can make?


void Serialize(const Call* call, char* buffer) { // called on one side
  memcpy(buffer, &call->function_enum, sizeof(Command));

  memcpy(buffer+32, (void*)call->args.arg1, sizeof(float));
  memcpy(buffer+64, (void*)call->args.arg2, sizeof(float));
  memcpy(buffer+96, (void*)call->args.arg3, sizeof(float));

  if (call->ret.arg1 != 0x0) 
    memcpy(buffer+128, (void*)call->ret.arg1, sizeof(float));
  if (call->ret.arg2 != 0x0) 
    memcpy(buffer+160, (void*)call->ret.arg2, sizeof(float));
  if (call->ret.arg3 != 0x0) 
    memcpy(buffer+192, (void*)call->ret.arg3, sizeof(float));
  if (call->ret.err != 0x0) 
    memcpy(buffer+224, (void*)call->ret.err, sizeof(float));

}; //This needs to be serialized, then sent, and then freed

void Deserialize(const char* buffer, Call *call) { // called on another side
  memcpy((void*)call->function_enum, buffer, sizeof(float));

  memcpy((void*)call->args.arg1, buffer+32, sizeof(float));
  memcpy((void*)call->args.arg2, buffer+64, sizeof(float));
  memcpy((void*)call->args.arg3, buffer+96, sizeof(float));

  memcpy((void*)call->ret.arg1, buffer+128, sizeof(float));
  memcpy((void*)call->ret.arg2, buffer+164, sizeof(float));
  memcpy((void*)call->ret.arg3, buffer+192, sizeof(float));

  memcpy((void*)call->ret.err, buffer+224, sizeof(float));


  call->function_enum = *(Command*)buffer; //typecast through *Command
  
  *(char*)call->args.arg1 = buffer+32; 


};


void Dispatcher(Call* call) { // Only going to be run on the MCU
  int* err = malloc(sizeof(float));
  int* ret1 = malloc(sizeof(float));
  int* ret2 = malloc(sizeof(float));
  int* ret3 = malloc(sizeof(float));
  switch(*(Command*)call->function_enum) {
    case read_sensor:
      *err = 0;
      //TODO: extern void read_sensor()
      //
      // Assign its output to ret and then reserialize to the other end .
      break;

    case run_motor:   //currently just running it at a certain speed
      extern int run_motor_fn(int motor_id, int pwm_frequency);
      *err = run_motor_fn(*(int*)call->args.arg1, *(int*)call->args.arg2);
      break;

    case pid_to_position:
      *err = 0;
      // TODO: 
      break;

    case finger_pos:
      *err = 0;
      // TODO:
      break;
  }

  call->ret.err = err;
}
